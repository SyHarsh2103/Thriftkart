...
const ForgotPassword = () => {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      setIsLoading(true);
      const res = await postData("/api/user/forgotPassword/send-otp", { email: formfields.email });
      if (res?.success) {
        context.setAlertBox({ open: true, error: false, msg: res?.message || "OTP sent!" });
        setStep(2);
      } else {
  ...
  const handleVerifyOtp = async () => {
    try {
      ...
      if (res?.success) {
        ...
        context.setAlertBox({ open: true, error: false, msg: res?.message || "OTP Verified!" });
      } else {
        setResetToken(null);
  ...
  const handleResetPassword = async (e) => {
    e.preventDefault();
    if (!canSubmitStep2) {
      context.setAlertBox({ open: true, error: true, msg: "Please fill fields correctly" });
      return;
    }
    if (!resetToken) {
      context.setAlertBox({ open: true, error: true, msg: "Verify OTP first" });
      return;
    }
    ...
    try {
      ...
      if (res?.success) {
        context.setAlertBox({ open: true, error: false, msg: res?.message || "Password reset!" });
    ...
    <section className="section signInPage">
      ...
      <div className="container">
        <div className="box card p-3 shadow border-0">
          ...
          <form className="mt-3" onSubmit={step === 1 ? handleSendOtp : handleResetPassword}>
            ...
            <Button type="submit" className="btn-blue col btn-lg btn-big w-100" disabled={isLoading}>
              {isLoading ? <CircularProgress size={24} /> : (step === 1 ? "Send OTP" : "Reset Password")}
            </Button>

            <div className="text-center mt-3">
              <Link to="/signIn" className="border-effect">Back to Login</Link>
            </div>
          </form>
        </div>
      </div>
