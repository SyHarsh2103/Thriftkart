...
const ProductUpload = () => {
  ...
  useEffect(() => {
    fetchDataFromApi("/api/subCat").then((res) => {
      setSubCatData(res.subCategoryList); // backend response { subCategoryList: [...] }
    });
  }, []);
...
>
  ...
  {subCatData
    ?.filter((sc) => sc.category?._id === categoryVal) // ✅ only show for selected category
    .map((subCat, index) => (
      <MenuItem
        className="text-capitalize"
        value={subCat._id}
        key={index}
        onClick={() => selectSubCat(subCat.subCat, subCat._id)}
      >
        {subCat.subCat}
      </MenuItem>
    ))}
